...
#this should be changed to use something like 80% of the current system's available rM. iS THERE A CALL TO GET CURRENT MAX RAM?
memory.limit(200000)
...
## Creating Binary Outcome Classes

SVM are designed to handle binary outcomes, however, our outcome is trinary (<30 >30 no). We are aware of 3 strategies for addressing this. These strategies require creating alternate binary outcomes for use in analysis. We will create 3 variables:

```{r}

PatientData$UnderNo <- 0
PatientData[PatientData$UnderNo != ">30",]$UnderNo <- 1

PatientData$OverNo <- 0
PatientData[PatientData$UnderNo != "<30",]$OverNo <- 1

PatientData$OverUnder <- 0
PatientData[PatientData$UnderNo != "NO",]$OverUnder <- 1

PatientData$readmit.B <- PatientData$readmitted
levels(PatientData$readmit.B) <- c("YES","YES","NO")
...
```{r}
svmfit = svm(readmitted ~ examide + acarbose + nateglinide + diag_1 , PatientData, "linear", cost = 10, scale = FALSE)
testData <- PatientData[c("readmitted","acarbose","diag_1")]

svmfit = svm(readmitted ~ acarbose + diag_1, data = testData[1:100,], kernel = "linear")
summary(PatientData$examide)
plot(svmfit, data = testData[1:100,])
predictions <- predict(svmfit)
predictionsTest <- predict(svmfit, testData[101:300,])
set.seed(1000)
trainingRows <- sample(nrow(PatientData), round(nrow(PatientData)*0.2,0))
testingRows <- 1:nrow(PatientData) %!in% trainingRows


AllDataSVMFit <- svm(readmitted ~ ., data = PatientData[trainingRows,], kernel = "linear")
trainPredictions <- predict(AllDataSVMFit)
testPredictions <-predict(AllDataSVMFit, PatientData[testingRows,])
```
